"""
Helper script to start the API MCP server with ngrok tunnel.
This will expose your local API MCP server to the internet via ngrok.
"""
...
def main():
    ...
    try:
        ...
        if server_process.poll() is not None:
            print("Error: API MCP server failed to start!")
            print("Make sure you have configured your .env file with API credentials.")
            return
        
        print(f"✓ API MCP Server started successfully on http://localhost:{MCP_SERVER_PORT}")
        ...
        if public_url:
            ...
            print(f"\n{'='*60}")
            print("\nYour API MCP server is now accessible from anywhere!")
        ...
        while True:
            ...
            if server_process.poll() is not None:
                print("\n⚠ API MCP server stopped unexpectedly!")
                break
